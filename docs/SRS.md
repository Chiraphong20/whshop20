...
| FR-03-2 | ระบบต้องคำนวณราคาสินค้าอัตโนมัติตามจำนวนที่สั่ง (ปลีก / ส่ง / ยกลัง) และแสดง Hint บอกลูกค้าว่าเพิ่มอีกกี่ชิ้นจะได้ราคาส่ง |
| FR-03-3 | ลูกค้าต้องกรอกชื่อ, ช่องทางติดต่อ (เบอร์โทร) และเลือกวิธีรับสินค้า (`DELIVERY` = จัดส่งถึงบ้าน หรือ `PICKUP` = รับเองที่ร้าน) |
| FR-03-4 | ถ้าเลือก `DELIVERY` ลูกค้าต้องกรอกที่อยู่จัดส่ง; ถ้าเลือก `PICKUP` ระบบจะบันทึก `address` = "รับเองที่ร้าน" อัตโนมัติ |
| FR-03-5 | ระบบต้องสร้าง Order ID รูปแบบ `ORDDD-MM-SEQ` ให้อัตโนมัติ |
...
| FR-03-8 | เมื่อสั่งซื้อสำเร็จ ถ้าลูกค้าอยู่ใน LIFF และ Login อยู่ ระบบต้องส่งข้อความยืนยันออเดอร์ผ่าน `liff.sendMessages()` ในฐานะลูกค้า |
...
| FR-08-6 | เมื่อสั่งซื้อสำเร็จ หากลูกค้าอยู่ใน LIFF Client ระบบต้องเรียก `liff.sendMessages()` เพื่อส่งข้อความยืนยันกลับไปในแชท LINE โดยอัตโนมัติ |
...
*เอกสารนี้จัดทำโดยอ้างอิงจาก Source Code จริงของระบบ ณ วันที่ 27 มีนาคม 2569*  
*เวอร์ชัน 1.1: เพิ่มเติม FR-03 (deliveryMethod, LIFF sendMessages, Price Hint) และ FR-08-6*
